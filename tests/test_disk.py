...
def test_new_run_creates_directories(tmp_run_root):
    """Test that creating a new run creates all expected directories."""
    Run(tmp_run_root, new=True)

    assert tmp_run_root.exists()
    assert (tmp_run_root / "checkpoint").exists()
    assert (tmp_run_root / "links").exists()
    assert (tmp_run_root / "inference").exists()


def test_existing_run_validates_structure(populated_run_root):
    """Test that an existing run validates its directory structure."""
    run = Run(populated_run_root, new=False)
...
def test_new_run_default_parameter(tmp_run_root):
    """Test that new parameter defaults to False."""
    # This should fail if the directory doesn't exist, confirming new=False is default
    with pytest.raises((FileNotFoundError, AssertionError)):
        Run(tmp_run_root)

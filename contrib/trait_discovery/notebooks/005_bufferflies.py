...
@app.cell
def _(collections, mo, pl, plt, sae_df, saev):
    def _(df: pl.DataFrame):
        ...
        for i, (layer, color, marker) in enumerate(
            zip(layers, saev.colors.ALL_RGB01, ("o", "^", "s", "x", "+", "o"))
        ):
            texts = []
            group = df.filter(
...
@app.cell
def _(clf_df, itertools, np, pl, plt, saev):
    def _(df: pl.DataFrame):
        x_col = "n_nonzero"
        y_col = "cls/test_acc"
        ...
        task_styles = {
            task: (color, marker)
            for task, color, marker in zip(
                tasks, itertools.cycle(saev.colors.ALL_RGB01), itertools.cycle(markers)
            )
        }
        ...
        for i, (layer, ax) in enumerate(zip(layers, axes)):
            for task, (color, marker) in task_styles.items():

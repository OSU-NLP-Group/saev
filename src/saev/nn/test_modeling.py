...
def test_factories():
    ...
    assert isinstance(modeling.get_activation(config.TopK()), modeling.TopK)
    assert isinstance(modeling.get_activation(config.BatchTopK()), modeling.BatchTopK)
...
def batch_topk_cfgs(draw):
    ...
    exp = draw(st.sampled_from([2, 4]))
    k = draw(st.sampled_from([1, 2, 4, 8]))
    return config.BatchTopK(d_vit=d_vit, exp_factor=exp, top_k=k)

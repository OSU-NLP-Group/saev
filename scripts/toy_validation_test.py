...
def _(Tree, json, torch):
    ...
    tree_dict = json.load(
        open("/Users/Beattie.74/Documents/Git Repos/saev/scripts/tree.json", "r")
    )
...
def _(DEVICE, D_MODEL, Dataset, np, torch, tree, vanilla_config):
    ...
    # Setup dataloader
    dataset = TreeDataset(
        tree, true_feats.cpu(), batch_size=200, num_batches=vanilla_config["n_steps"]
    )
...
def _(DEVICE, np, torch):
    ...
    def get_latent_perm(sae, tree_ds, include_all_latents=False):
        ...
        true_acts = true_acts / true_acts.max(dim=0, keepdim=True).values.clamp(
            min=1e-10
        )

        with torch.no_grad():
            sae_acts, x_hat = sae(x)
            sae_acts = sae_acts / sae_acts.max(dim=0, keepdim=True).values.clamp(
                min=1e-10
            )
        ...
        row_ind, col_ind = row_ind, col_ind = linear_sum_assignment(
            cost_matrix.detach().cpu().numpy().T
        )
        leftover_features = np.array(
            list(
                {i for i in range(sims.shape[0]) if sae_acts.max(dim=0).values[i] > 0}
                - set(col_ind)
            )
        ).astype(int)
...
def _(
    ...
    from saev import config, helpers, nn
    from saev.training import BatchLimiter, Warmup, make_saes
    ...
    def train() -> tuple[torch.nn.ModuleList, torch.nn.ModuleList]:
        ...
        dataset = TreeDataset(
            ...
            num_batches=vanilla_config["n_steps"],
        )
        ...
        for batch in helpers.progress(dataloader, every=20):
            ...
            for sae, objective in zip(saes, objectives):
                if isinstance(objective, nn.objectives.MatryoshkaObjective):
                    ...
                    prefix_preds, f_x = sae.matryoshka_forward(
                        acts_BD, matryoshka_config["n_prefixes"]
                    )
...
@app.cell(hide_code=True)
def _(matryoshka_perm, matryoshka_sae, true_feats, vanilla_perm, vanilla_sae):
    ...
    matryoshka_cosine = (
        F.normalize(matryoshka_feats, dim=1)[matryoshka_perm]
        @ F.normalize(true_feats, dim=1).T
    )
    vanilla_cosine = (
        F.normalize(vanilla_feats, dim=1)[vanilla_perm]
        @ F.normalize(true_feats, dim=1).T
    )
    ...
    # matryoshka_enc_true = (
    #    F.normalize(matryoshka_enc, dim=1)[matryoshka_perm]
    #    @ F.normalize(true_feats, dim=1).T
    # )
    # vanilla_enc_true = (
    #    F.normalize(vanilla_enc, dim=1)[vanilla_perm] @ F.normalize(true_feats, dim=1).T
    # )

def make_cfgs():
    ...
    # Butterflies uses a single shards directory for both train and test
    ...
    # Known mimic pairs from logbook.md (erato subspecies vs melpomene subspecies)
    # Each pair co-evolved in the same geographic region
    mimic_pairs = [
        ("lativitta", "malleti"),
        ("cyrbia", "cythera"),
        ("notabilis", "plesseni"),
    ...
    # Build LabelGrouping tasks for each mimic pair
    tasks = []
    for erato_ssp, melp_ssp in mimic_pairs:
        # Match class names that contain the subspecies name e.g., "lativitta" matches "Heliconius erato ssp. lativitta" and hybrids
        tasks.append(
            LabelGrouping(
    ...
    for layer, ids in run_ids.items():
        for run_id in ids:
            for agg in [PatchAgg.MEAN, PatchAgg.MAX]:
